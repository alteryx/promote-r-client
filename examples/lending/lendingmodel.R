...
bad_indicators <- c("Late (16-30 days)", "Late (31-120 days)",
                    "Default", "Charged Off")
...
my.glm <- glm(I(is_bad == TRUE) ~ last_fico_range_low +
                last_fico_range_high +
                is_rent + loan_amnt, data = train
...
my.glm$data <- NULL
my.glm$y <- NULL
my.glm$linear.predictors <- NULL
my.glm$weights <- NULL
my.glm$fitted.values <- NULL
my.glm$model <- NULL
my.glm$prior.weights <- NULL
my.glm$residuals <- NULL
my.glm$effects <- NULL
...
TESTDATA <- jsonlite::fromJSON('{
  "loan_amnt": 54000,
  "home_ownership": "RENT",
  "annual_inc": 400000,
  "inq_last_6mths": 0,
  "last_fico_range_high": 700,
  "last_fico_range_low": 640
}')
